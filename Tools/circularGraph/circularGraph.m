  ...
  methods
    function this = circularGraph(adjacencyMatrix,varargin)
      ...
      for i = 1:length(adjacencyMatrix)
        ...
        %}
        markersize = get(gca,'Position'); units = get(gca,'Units');
        if units=="pixels"
            markersize = 5.4*markersize(3)/length(adjacencyMatrix);
        elseif units=="inches"
            markersize = 72*5.4*markersize(3)/length(adjacencyMatrix);
        elseif units=="normalized"
            markersize = 12000*markersize(3)/length(adjacencyMatrix);
        end
        this.Node(i).NodeMarker.MarkerSize = markersize;
       

      end
      % clear normalselfconn
      
      % PB ADDED FOR TESTING
      ...
      vp = zeros(size(v)); vn = zeros(size(v));
      vp(v>0) = v(v>0); vn(v<0) = v(v<0);
%       al = zeros(size(v));
%       alp = normalize(vp,'range',[0.1,1]);
%       aln = normalize(-vn,'range',[0.1,1]);
      ...
      % don't bother with this; alpha is enough
      %lineWidth = 0.9*ones(size(lineWidth));
      ...
      for j = 1:length(v)
        [~,lineorder] = sort(lineWidth,'ascend');
        i = lineorder(j);
